...
output "worker_servers" {
  description = "All worker server details"
  value = {
    for server_key, server in hcloud_server.worker_nodes : server_key => {
      id          = server.id
      name        = server.name
      ipv4        = server.ipv4_address
      ipv6        = server.ipv6_address
...
output "worker_ipv4_addresses" {
  description = "IPv4 addresses of all worker servers"
  value       = [for server in hcloud_server.worker_nodes : server.ipv4_address]
}

output "worker_names" {
  description = "Names of all worker servers"
  value       = [for server in hcloud_server.worker_nodes : server.name]
}